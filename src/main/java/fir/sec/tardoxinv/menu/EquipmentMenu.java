...
public class EquipmentMenu extends AbstractContainerMenu {
    ...
    public EquipmentMenu(int id, Inventory playerInventory, int backpackWidth, int backpackHeight) {
        super(fir.sec.tardoxinv.menu.ModMenus.EQUIPMENT_MENU.get(), id);
        ...
        for (int row = 0; row < 2; row++)
            for (int col = 0; col < 2; col++) {
                int gi = col + row * 2;
                addSlot(new GridSlot(base2x2, gi, baseInvX + col * SLOT_SIZE, baseInvY + row * SLOT_SIZE, GridSlot.Storage.BASE));
            }
    ...
    @Override public void broadcastChanges() { super.broadcastChanges(); }
